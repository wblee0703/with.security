...
export default function WorkLogTab({ onTriggerToast }) {
  ...
  const handleCopySinglePastLog = async (pastLog) => {
    ...
    try {
      ...
      const newLogItem = {
        ...
        subCategory: pastLog.subCategory || pastLog.sub_category || '',
        sub_category: pastLog.subCategory || pastLog.sub_category || '',
        dueDate: pastLog.dueDate || pastLog.due_date || '',
        due_date: pastLog.dueDate || pastLog.due_date || '',
        date: selectedDate,
        title: pastLog.title,
        details: pastLog.details || pastLog.tasksDone || pastLog.tasks_done || '',
        tasks_done: pastLog.details || pastLog.tasksDone || pastLog.tasks_done || '',
  ...
  const isLogVisibleToCurrentUser = (log, user = currentUser) => {
    if (!user || !user.username) return false;
    if (!log) return false;

    // 1. 해당 계정 아이디(username / writer_id) 일치 시 최우선 노출 (대소문자 무시)
    const logWriter = String(log.writer_id || log.writerId || log.authorUsername || log.author_username || log.username || log.userId || '').trim().toLowerCase();
